...
| `crates/fluxheim-server/src/native_http1_proxy_tests.rs` | 991 | Native HTTP/1 proxy parity tests temporarily group plain, TLS, mTLS, failover, pooling, and config-gate fixtures for the `1.6.14` upstream TLS cutover slice. | Split into plain proxy, TLS, mTLS, failover, and config-gate test modules after the native proxy API stabilizes. |
