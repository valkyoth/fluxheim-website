...
| `crates/fluxheim-server/src/native_http1_proxy_tests.rs` | 580 | Native HTTP/1 proxy parity tests temporarily group plain, TLS, mTLS, failover, pooling, round-robin, weighted round-robin, and config-gate fixtures for the native upstream cutover slices. | Split the remaining plain failover/round-robin tests from TLS, pooling, and config-gate modules after the native proxy API stabilizes. |
| `crates/fluxheim-server/src/native_http1_tls.rs` | 854 | Native upstream TLS connector covers rustls/OpenSSL, trust roots, SNI, hostname policy, mTLS material, and file-safety checks during the staged `1.6.14` cutover. | Split rustls, OpenSSL, file loading, and hostname-policy helpers before production native proxy cutover. |
| `crates/fluxheim-server/src/native_http1.rs` | 637 | Native downstream HTTP/1 listener, TLS accept loop, connection budget, request parsing, and response framing remain grouped during the `1.6.19` TLS listener preview. | Split plain listener, TLS accept loops, connection serving, and response framing before production native HTTP cutover. |
