...
curl -fsS "${base}/de/" | grep -q "Herunterladen v1.7.3"
curl -fsS "${base}/ch/" | grep -q "Herunterladen v1.7.3"
curl -fsS "${base}/fr/" | grep -q "Télécharger v1.7.3"
curl -fsS "${base}/no/" | grep -q "Last ned v1.7.3"
curl -fsS "${base}/nl/" | grep -q "Download versie 1.7.3"
curl -fsS "${base}/fi/" | grep -q "Lataa v1.7.3"
curl -fsS "${base}/is/" | grep -q "Sækja v1.7.3"
curl -fsS "${base}/da/" | grep -q "Hent v1.7.3"
curl -fsS "${base}/es/" | grep -q "Descargar v1.7.3"
curl -fsS "${base}/pt/" | grep -q "Transferir v1.7.3"
curl -fsS "${base}/et/" | grep -q "Laadi alla v1.7.3"
curl -fsS "${base}/lv/" | grep -q "Lejupielādēt v1.7.3"
curl -fsS "${base}/el/" | grep -q "Λήψη v1.7.3"
curl -fsS "${base}/it/" | grep -q "Scarica v1.7.3"
curl -fsS "${base}/lt/" | grep -q "Atsisiųsti v1.7.3"
curl -fsS "${base}/hr/" | grep -q "Preuzmi v1.7.3"
curl -fsS "${base}/cs/" | grep -q "Stáhnout v1.7.3"
curl -fsS "${base}/bs/" | grep -q "Preuzmi v1.7.3"
curl -fsS "${base}/bg/" | grep -q "Изтегли v1.7.3"
curl -fsS "${base}/ro/" | grep -q "Descarcă v1.7.3"
curl -fsS "${base}/pl/" | grep -q "Pobierz v1.7.3"
curl -fsS "${base}/ru/" | grep -q "Скачать v1.7.3"
curl -fsS "${base}/ja/" | grep -q "ダウンロード v1.7.3"
