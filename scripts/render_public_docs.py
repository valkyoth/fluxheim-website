...
DOCS_CONTENT = ROOT / "content/docs"
...
def page_body(page: str) -> str:
    return (DOCS_CONTENT / page).read_text(encoding="utf-8")
...
{site_header}
  <div class="flex pt-16">
    <aside class="hidden md:block sticky top-16 max-h-[calc(100vh-4rem)] self-start w-64 lg:w-72 flex-none border-r border-gray-800/60 bg-gray-950/80 overflow-y-auto z-20">
...
def main() -> int:
    docs = ROOT / "docs"
    for page, (title, intro) in PAGE_META.items():
        (docs / page).write_text(render(page, title, intro, page_body(page)), encoding="utf-8")
    return 0
