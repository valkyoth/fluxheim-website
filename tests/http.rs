...
async fn renders_default_english_home() {
    ...
    assert!(body.contains("Download v1.6.36"));
...
async fn locale_prefixes_preserve_legacy_pages() {
    ...
    assert!(de_body.contains("Herunterladen v1.6.36"));
    assert!(de_body.contains("v1.6.0 – v1.6.36"));
    ...
    assert!(ch_body.contains("Herunterladen v1.6.36"));
    ...
    assert!(sv_body.contains("Ladda ner v1.6.36"));
    ...
    assert!(nb_body.contains("Last ned v1.6.36"));
    ...
    assert!(nl_body.contains("Download versie 1.6.36"));
    ...
    assert!(fi_body.contains("Lataa v1.6.36"));
    ...
    assert!(is_body.contains("Sækja v1.6.36"));
    ...
    assert!(da_body.contains("Hent v1.6.36"));
    ...
    assert!(es_body.contains("Descargar v1.6.36"));
    ...
    assert!(pt_body.contains("Transferir v1.6.36"));
    ...
    assert!(et_body.contains("Laadi alla v1.6.36"));
    ...
    assert!(lv_body.contains("Lejupielādēt v1.6.36"));
    ...
    assert!(el_body.contains("Λήψη v1.6.36"));
    ...
    assert!(it_body.contains("Scarica v1.6.36"));
    ...
    assert!(lt_body.contains("Atsisiųsti v1.6.36"));
    ...
    assert!(hr_body.contains("Preuzmi v1.6.36"));
    ...
    assert!(cs_body.contains("Stáhnout v1.6.36"));
    ...
    assert!(bs_body.contains("Preuzmi v1.6.36"));
    ...
    assert!(bg_body.contains("Изтегли v1.6.36"));
    ...
    assert!(ro_body.contains("Descarcă v1.6.36"));
    ...
    assert!(pl_body.contains("Pobierz v1.6.36"));
    ...
    assert!(ru_body.contains("Скачать v1.6.36"));
    ...
    assert!(ja_body.contains("ダウンロード v1.6.36"));
    ...
    assert!(ko_body.contains("v1.6.36 다운로드"));
    ...
    assert!(hu_body.contains("v1.6.36 letöltése"));
...
async fn english_variant_prefixes_preserve_english_content() {
    ...
    assert!(gb_body.contains("Download v1.6.36"));
...
async fn locale_prefixes_apply_runtime_translations() {
    ...
    assert!(fr_body.contains("Télécharger v1.6.36"));
...
async fn download_outbound_redirects_only_known_artifacts() {
    let artifact = "fluxheim-1.6.36-full-x86_64-linux.tar.gz";
    let (status, headers, _body) = request(&format!("/out/download/{artifact}?locale=en-EU")).await;
    assert_eq!(status, StatusCode::TEMPORARY_REDIRECT);
    assert_eq!(
        headers[header::LOCATION],
        format!("https://github.com/valkyoth/fluxheim/releases/download/v1.6.36/{artifact}")
    );

    let (unknown_status, _headers, body) =
        request("/out/download/fluxheim-1.6.36-private-token.tar.gz").await;
    assert_eq!(unknown_status, StatusCode::NOT_FOUND);
...
async fn rendered_pages_keep_links_and_inject_click_beacon() {
    ...
    assert!(
        body.contains(
            r#"href="https://github.com/valkyoth/fluxheim/releases/download/v1.6.36/fluxheim-1.6.36-full-x86_64-linux.tar.gz""#
        )
    );
