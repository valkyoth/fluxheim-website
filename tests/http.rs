...
async fn renders_default_english_home() {
    ...
    assert!(body.contains("Download v1.7.7"));
...
async fn locale_prefixes_preserve_legacy_pages() {
    ...
    assert!(de_body.contains("Herunterladen v1.7.7"));
    assert!(de_body.contains("v1.7.7"));
    ...
    assert!(ch_body.contains("Herunterladen v1.7.7"));
    ...
    assert!(sv_body.contains("Ladda ner v1.7.7"));
    ...
    assert!(nb_body.contains("Last ned v1.7.7"));
    ...
    assert!(nl_body.contains("Download versie 1.7.7"));
    ...
    assert!(fi_body.contains("Lataa v1.7.7"));
    ...
    assert!(is_body.contains("Sækja v1.7.7"));
    ...
    assert!(da_body.contains("Hent v1.7.7"));
    ...
    assert!(es_body.contains("Descargar v1.7.7"));
    ...
    assert!(pt_body.contains("Transferir v1.7.7"));
    ...
    assert!(et_body.contains("Laadi alla v1.7.7"));
    ...
    assert!(lv_body.contains("Lejupielādēt v1.7.7"));
    ...
    assert!(el_body.contains("Λήψη v1.7.7"));
    ...
    assert!(it_body.contains("Scarica v1.7.7"));
    ...
    assert!(lt_body.contains("Atsisiųsti v1.7.7"));
    ...
    assert!(hr_body.contains("Preuzmi v1.7.7"));
    ...
    assert!(cs_body.contains("Stáhnout v1.7.7"));
    ...
    assert!(bs_body.contains("Preuzmi v1.7.7"));
    ...
    assert!(bg_body.contains("Изтегли v1.7.7"));
    ...
    assert!(ro_body.contains("Descarcă v1.7.7"));
    ...
    assert!(pl_body.contains("Pobierz v1.7.7"));
    ...
    assert!(ru_body.contains("Скачать v1.7.7"));
    ...
    assert!(ja_body.contains("ダウンロード v1.7.7"));
    ...
    assert!(ko_body.contains("v1.7.7 다운로드"));
    ...
    assert!(hu_body.contains("v1.7.7 letöltése"));
...
async fn english_variant_prefixes_preserve_english_content() {
    ...
    assert!(gb_body.contains("Download v1.7.7"));
...
async fn locale_prefixes_apply_runtime_translations() {
    ...
    assert!(fr_body.contains("Télécharger v1.7.7"));
...
async fn advanced_uses_page_specific_translations() {
    ...
    assert!(de_body.contains("Proxy-Wasm-ABI-Kompatibilitätsvorschau"));
    ...
    assert!(fr_body.contains("compatibilité avec l’ABI Proxy-Wasm"));

    let (ja_status, _headers, ja_body) = request("/ja/docs/advanced").await;
    assert_eq!(ja_status, StatusCode::OK);
    assert!(ja_body.contains("Proxy-Wasm ABI 互換 preview"));
    assert!(ja_body.contains("決定的に fail-closed"));
}
...
async fn release_note_artifacts_are_served() {
    let (status, headers, body) = request("/fr/docs/releases/RELEASE_NOTES_1.7.7.md").await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(headers["content-type"], "text/markdown; charset=utf-8");
    assert!(body.contains("# Fluxheim 1.7.7 Release Notes"));
    assert!(body.contains("wasm-proxy-abi"));
}
...
async fn download_outbound_redirects_only_known_artifacts() {
    ...
    assert_eq!(
        headers[header::LOCATION],
        format!("https://github.com/valkyoth/fluxheim/releases/download/v1.7.7/{artifact}")
    );
