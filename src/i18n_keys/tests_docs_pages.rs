...
fn applies_expanded_docs_keys_only_to_docs_pages() {
    let site = Site::load().expect("site loads");
    let de = site.locale("de-DE").expect("German locale");
    let paragraph = "Fluxheim 1.7.10 makes every migration family independently testable, keeps the complete Wasm smoke test in the release gate, and verifies that guest-controlled symbolic IDs are rejected safely. In-process host callbacks remain limited to finite, non-blocking operations.";
    ...
    let docs_html = apply_shared_keys(de, docs, "1.7.10");
    assert!(docs_html.contains("jede Migrationsfamilie unabhängig testbar"));
    ...
    let non_docs = apply_shared_keys(de, format!("<p>{paragraph}</p>"), "1.7.10");
