...
fn applies_stable_download_keys_for_release_page_copy() {
    ...
    let translated = apply_shared_keys(de, html, "1.6.35");
...
fn applies_stable_changelog_keys_only_on_changelog_page() {
    ...
    let translated = apply_shared_keys(de, html, "1.6.35");
    let unrelated = apply_shared_keys(de, "Released June 23, 2026".to_owned(), "1.6.35");
...
fn applies_public_docs_guide_keys_on_docs_pages() {
    ...
    let de_html = apply_shared_keys(de, html.clone(), "1.6.35");
    let fr_html = apply_shared_keys(fr, html, "1.6.35");
    let unrelated = apply_shared_keys(de, ">External PHP-FPM pool<".to_owned(), "1.6.35");
